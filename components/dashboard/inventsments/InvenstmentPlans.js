...
export default function InvestmentPlans() {
  ...
  const handlePlanChange = async (min, index, plan) => {
    ...
    try {
      const balance = parseFloat(details.tradingBalance) || 0;
      const planPrice = parseFloat(min) || 0;
      
      if (balance >= planPrice) {
        // Make an API request to purchase the plan
        const response = await axios.post("/plan/api", { plan, email, amount: planPrice });

        // Check if the purchase was successful (you may need to adjust this based on your API response)
        if (response.data.success) {
          setDetails((prevDetails) => ({
            ...prevDetails,
            tradingBalance: prevDetails.tradingBalance - planPrice,
